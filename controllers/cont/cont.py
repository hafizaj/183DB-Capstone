...
class ArmController(Robot):
    def __init__(self):
        ...
        shoulderMotor = self.getDevice('ShoulderHingeMotor')
        elbowMotor = self.getDevice('ElbowMotor')
        wristMotor = self.getDevice('WristMotor')
        self.motors = [shoulderMotor, elbowMotor, wristMotor]

        # Connect the base and storage unit
        # Linking bas
        self.base_storage_tx = self.getDevice('base_storage_tx')
        self.base_storage_rx = self.getDevice('base_storage_rx')
        
        # Initialize and enable device tags
        self.dsEndEffector = self.getDevice('dsEndEffector')
        self.endEffectorLocation = self.getDevice('endEffectorLocation')
        self.ShoulderSensor = self.getDevice('ShoulderSensor')
        self.ElbowSensor = self.getDevice('ElbowSensor')
        self.WristSensor = self.getDevice('WristSensor')
        ...
        # Storage Unit
        self.storageCont = StorageController(self)
        
        # End Effector Trajectory
        # Connector Devices
        self.gripperTool = GripperController(self)
        self.frontConnector = self.getDevice('frontConnector')
        self.frontConnector.enablePresence(self.timeStep)
        
        # Vertical Slider
        self.verticalSlider = self.getDevice('verticalSlider')
        self.verticalSliderPosition = self.getDevice('verticalSliderPosition')
    ###################### Motor Control ######################
    ...
    def generateVelocities(self, theta1_, theta2_, theta3_, runtime_):
        '''
    ...
    def simulateMotion(self, theta1_, theta2_, theta3_, runtime_):
        '''
        ...
        while(np.abs(self.verticalSliderPosition.getValue()- VERTICAL_SLIDER_POSITION)>SLIDER_THRESHOLD):
            self.step(self.timeStep)
            self.verticalSlider.setPosition(VERTICAL_SLIDER_POSITION)
        ...
        # Move up
        while(np.abs(self.verticalSliderPosition.getValue()- 0)>SLIDER_THRESHOLD):
            self.step(self.timeStep)
            self.verticalSlider.setPosition(0)
        ...
        self.generateVelocities(np.rad2deg(initial_positions[0]), 
            ...
            np.rad2deg(initial_positions[2]), runtime_)

        # activate the device
        # Release
        # Move down 0.053
        while(np.abs(self.verticalSliderPosition.getValue()- 0.053)>0.1):
            self.step(self.timeStep)
            self.verticalSlider.setPosition(- 0.053)
        ...
        # self.endEffectorTrajectory = np.array(traj_li).round(4)       
        # self.jointPositionTrajectory = np.array(joint_li).round(4)         
...
# t1,t2,t3 = inv_kinematics(0.07184205,0.67453127, 135)
# controller.actualTrajectories(t1,t2,t3,2)
# controller.simulateMotion(0,0,0,6)
